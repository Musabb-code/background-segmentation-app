...
## Phase 6 — Gallery & export studio (PLAN §8.4.11)

> Blueprint: [GALLERY_EXPORT_PLAN.md](GALLERY_EXPORT_PLAN.md)

- [x] **Task G0** — Adopt §8.4.11 into PLAN.md + LOG

- [x] **Task G1** — `crop_utils.dart` + `export_utils.dart` + unit tests

- [x] **Task G2** — Extract `export_preview_dialog.dart` + `editor_provider.dart`

- [x] **Task G3** — Gallery editor screen + `/editor` route + Home button

- [x] **Task G4** — Export format picker (PNG / JPG white / WebP)

- [x] **Task G5** — Auto-crop toggle + pipeline hook

- [x] **Task G6** — E-commerce white BG preset on Home

- [x] **Task G7** — Batch mode (up to 20 images)

- [x] **Task G8** — QA + APK verify per [APK_PHONE_RUNBOOK.md](APK_PHONE_RUNBOOK.md) (APK built; §4.4 device checklist pending manual run)

**Prompt footer (Phase 6):**
```
Stay inside docs/PLAN.md §8.4.11. Read docs/GALLERY_EXPORT_PLAN.md for the active prompt. MANDATORY: read .cursor/skills/ponytail/SKILL.md. Apply Ponytail YAGNI. Update docs/LOG.md. Mark Task G* complete in docs/TASKS.md.
```
