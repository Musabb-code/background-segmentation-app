...
/// PLAN §8.8 — splash → login → home with mocked API (widget-level integration).
void main() {
  ...
  testWidgets('splash → login → home', (tester) async {
    ...
    // Splash waits up to splashMaxMs then goes to login (no token).
    await tester.pump(const Duration(milliseconds: AppConstants.splashMaxMs + 100));
